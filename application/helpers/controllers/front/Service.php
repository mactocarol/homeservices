...
//                                 'promo_code'=>$code
    ...
    public function finalize(){
        ...
        $field_array = array('user_id','cat_id','zipcode','plan_book','helper','job_desc','first_item','second_item','thired_item','select_date','select_time','emailid','booking_status','booking_date','status','plan_hr_bathroom','plan_hr_badroom','livingroom','hour_prc','first_stuff');
		...
		try {            
			...
			require_once(APPPATH.'libraries/stripe/init.php');//or you
                //        Stripe::setApiKey("sk_test_xDQkMmdedbWXjlWi3CeRGMDc"); //Replace with your Secret Key
                        \Stripe\Stripe::setApiKey('sk_test_xDQkMmdedbWXjlWi3CeRGMDc');
                     //   print_r($_POST);exit;
//                        $email = $_POST['email'];
                      $plan = $_POST['plan'];
//                        $amount = $_POST['amount'];
...
//                       $end_date = date('Y-m-d H:i:s', $endTime);
                  ...
                  //  $rest = Stripe_Charge::retrieve($charge->id);
                    ...
                    $rest = \Stripe\Charge::retrieve($charge->id);
                    //print_r($rest);exit;
