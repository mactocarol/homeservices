    ...
    public function add_category() {
       // echo "hello";die;
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if($_POST['professionals'][0] != ''){                
                $professionals_recurr = $_POST['professionals'];
                $hours = $_POST['hours'];            
                ...
                $professionals_fixed = $_POST['professionals_fixed'];
                $hours = $_POST['hours_fixed'];            
                $fixed_price = (!empty($_POST['fixed_price'])) ? $_POST['fixed_price'] : '';
                $unit = $_POST['unit_fixed'];
            }
            ...
            if(isset($professionals_recurr)){
                for ($fs = 0; $fs < count($professionals_recurr); $fs++) {
                    $feedata[] = array('professionals' => $professionals_recurr[$fs], 'unit' => $unit[$fs], 'hours' => $hours[$fs], 'weekly_price' => $weekly_price[$fs],'biweekly_price' => $biweekly_price[$fs],'monthly_price' => $monthly_price[$fs]);
                }  
            }else{
                for ($fs = 0; $fs < count($professionals_fixed); $fs++) {
                    $feedata[] = array('professionals' => $professionals_fixed[$fs], 'unit' => $unit[$fs], 'hours' => $hours[$fs], 'fixed_price' => $fixed_price[$fs]);
                }  
            }
...
//            $final_stuff = json_encode($feedata1);
            ...
            if($extra[0] != ''){
                ...
                $extradataset_new = json_encode($extradata_new);
            }else{
                $extradataset_new = '';
            }
    ...
    public function edit_cat() {        
        ...
        $get_field = array('cat_parent_id', 'cat_name', 'is_active', 'cat_picture', 'cat_id', 'cat_desc', 'meta_title', 'meta_desc', 'meta_tag', 'cat_order', 'cat_attribute', 'req_message', 'stuff','hourly_rate','required_field_1','stuff_1','required_field_2','staff_2','required_field_3','staff_3','extra');
        ...
        if ($_POST) {
            ...
            if($fixed_price != ''){
                for ($fs = 0; $fs < count($professionals); $fs++) {
                    $feedata[] = array('professionals' => $professionals[$fs], 'unit' => $unit[$fs], 'hours' => $hours[$fs], 'fixed_price' => $fixed_price[$fs]);
                }  
            }else{
                for ($fs = 0; $fs < count($professionals); $fs++) {
