                                ...
                                <tbody>
                                    ...
                                    if ($records != 'no') {
                                        foreach ($records as $rec) {
                                            ...
                                            <tr>
                                                <td><?php echo $cnt; ?></td>
                                                <td><?php echo ucfirst($rec->page_title); ?></td>
<!--                                                <td><?php echo substr($rec->page_content,0,10); ?></td>-->
                                                <td><?php echo date('d M Y',strtotime($rec->update_date)); ?></td>
                                                <td id="statuscell_<?php echo $rec->page_id; ?>">
                                                <?php
                                                if ($rec->is_active == 1) {
                                                    ?>
                                                        <span id="span_active_<?php echo $rec->page_id; ?>" class="label label-success">Published</span> 
                                                        <?php
                                                    } else {
                                                        ?>
                                                        <span id="span_block_<?php echo $rec->page_id; ?>" class="label label-danger">Unpublished</span> 
                                                        <?php
                                                    }
                                                    ?>
                                                </td>
                                                <td>
                                                    <a href="<?php echo base_url(); ?>EditPageDetails/<?php echo ci_enc($rec->page_id); ?>" id='<?php echo $rec->page_id; ?>' title="Edit details"><i class="fa fa-edit"></i></a>&nbsp;&nbsp;
                                                    <a href="<?php echo base_url(); ?>DeletePage/<?php echo ci_enc($rec->page_id); ?>" id='<?php echo $rec->page_id; ?>' title="Delete"><i class="fa fa-trash-o"></i></a>
