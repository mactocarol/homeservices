    ...
    <div class="row">
        <div class="col-lg-12">
            <section class="panel">
                ...
                <?php
                $title = $fetch_recored->banner_title;
                $status = $fetch_recored->is_active;
                $image = $fetch_recored->banner_image;
                ?>
                <div class="panel-body"> <span style="color:red"><?php echo validation_errors(); ?> <?php echo $this->session->flashdata('Register_error'); ?></span>
                    <form role="form" class="form-horizontal" action="" method="post" enctype="multipart/form-data"  name="myForm" id="myForm">
                        <div class="form-group">
                            <label class="control-label col-sm-4">Banner Image<?php echo ASTRIK; ?></label>
                            <div class="col-sm-7">
                                <?php if(isset($image)){ ?>
                                <img id="banner_image_view" alt="" src="<?php echo MEDIAURL; ?>baner/<?php echo $image; ?>" style="width:20%; height:20%; margin-bottom:2px;"/> 
                                <input type="hidden" name="pre_img" value="<?php echo $image; ?>" />
                                <?php }else{ ?>
                                <img src="<?php echo MEDIAURL;?>images/no_image.gif" alt="" /> 
                                <?php } ?>
                                <input type="file" name="banner_img" id="banner_img" class="form-control" required="required">
                            </div>
                        </div>
                        <div class="form-group" id="category_section" style="<?php //echo $category_display; ?>">
                            <label class="control-label col-sm-4">Banner Title<?php echo ASTRIK; ?></label>
                            <div class="col-sm-7">
                                <input type="text" name="banner_title" id="banner_title" value="<?php echo $title; ?>"class="form-control" required="required">
                            </div>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="col-sm-offset-4 col-sm-8">
                                <?php
                                $banner_id = (empty($banner_detail['banner_id'])) ? '' : $banner_detail['banner_id'];
                                ?>
